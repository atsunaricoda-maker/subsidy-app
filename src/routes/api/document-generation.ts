...
【出力ルール - 厳守】
1. ★★★必ず日本語で出力すること（英語禁止）★★★
2. 文字数は${Math.floor(section.max_chars * 0.8)}〜${section.max_chars}文字を目標
3. セクション番号やタイトルは出力しない（内容のみ）
4. ★マークダウン記法は絶対禁止★（*、**、#、-、などの記号を装飾目的で使わない）
5. 箇条書きは「・」のみ使用可（*, -, などは禁止）
6. 上記「記載禁止事項」に該当する内容は絶対に書かない
7. 他セクションとの重複を避け、このセクション固有の内容のみ記載
8. ★ヒアリング回答にない情報は絶対に書かない★
9. ★文章は最後まで完結させること（途中で切れない）★
